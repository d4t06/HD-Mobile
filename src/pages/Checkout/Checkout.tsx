   ...
   const handleSubmit = async () => {
      try {
         ...
         const orderItems = cart.items.map((item) => {
            const color = item.product_data.colors_data.find(
               (cl) => cl.id === item.color_id
            );
            const storage = item.product_data.storages_data.find(
               (s) => s.id === item.storage_id
            );
            ...
            return {
               ...
               slug:
                  item.product_data.category_data.category_ascii +
                  "/" +
                  item.product_ascii,
