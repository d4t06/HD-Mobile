...
const PRICE_FIELDS: FieldType = ["Label", { label: "From", placeholder: "1 = 1.000.000đ" }, { label: "To", placeholder: "1 = 1.000.000đ" }];
...
export default function PriceRangeGroup({ categories }: Props) {
  ...
  const handleAddPriceRange = async (data: Record<string, string>, type: "Add" | "Edit") => {
    ...
    if (type === "Edit") {
      if (data === undefined || curPriceIndex.current === undefined || curCategory.price_ranges === undefined) {
  ...
  const renderModal = useMemo(() => {
    ...
    switch (openModalTarget.current) {
      case "add-price":
        ...
        return <AddItemMulti loading={apiLoading} title="Add price range" fields={PRICE_FIELDS} cb={(value) => handleAddPriceRange(value, "Add")} setIsOpenModal={setIsOpenModal} />;
      case "edit-price":
        if (curPriceIndex.current === undefined || curCategory === undefined || curCategory.price_ranges == undefined) return <h1>Index not found</h1>;
      ...
      case "delete-price":
        if (curPriceIndex.current === undefined || curCategory?.price_ranges == undefined) return <h1>Index not found</h1>;
