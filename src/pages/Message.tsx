...
export default function Message() {
   ...
   const { deleteComment, apiLoading, state, setState } = useComment({ admin: true, setIsOpenModal });
         ...
         case "Delete-Comment":
            return (
               <ConfirmModal
                  ...
                  callback={() => deleteComment(curComment, curCommentIndex.current as number, state, setState)}
                  loading={apiLoading}
   ...
   const commentCta = (index: number) => (
      <>
         <Button onClick={() => handleOpenModal("Add-Reply", index)} primary className={classes.button}>
            <i className="material-icons text-[15px]">reply</i>
            <span className="ml-[4px]">Reply</span>
         </Button>

         <Button onClick={() => handleOpenModal("Delete-Comment", index)} primary className={classes.button}>
   ...
   return (
      <div className="">
         ...
         {status === "success" && !comments.length && <p className="text-[16px] text-[#333]">Nothing to show</p>}
