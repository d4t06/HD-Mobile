...
function EditGroup({ attrList }: Props, ref: Ref<ConfigRef>) {
  const { combines_data, product_name_ascii, product_name, sliders_data, setIsChange, colors_data, storages_data } = useProductContext();
          ...
          <div className={`bg-[#fff] rounded-[8px] p-[20px] space-y-[20px]`}>
            {storages_data.map((storageItem, i) =>
              colors_data.map((colorItem, j) => {
                const { existCombine, isExits } = getExistCombine(combines_data, colorItem, storageItem, product_name_ascii);
                ...
                return (
                  <div key={key} className="flex justify-center items-center">
                    ...
                    <InputGroup ref={(ref) => (combineRefs.current[index] = ref!)} initCombine={existCombine} isExist={isExits} />
                  </div>
                );
              })
            )}
        ...
        {!!storages_data.length && (
          <div className={`${classes.container} flex-col`}>
            {attrList.map((attr, index) => {
              return <AttributeGroup ref={(ref) => (attributeRefs.current[index] = ref!)} catAttr={attr} key={index} />;
            })}
          </div>
        )}
      </div>
