...
type ModelTarget = "add-storage" | "edit-storage" | "add-color" | "edit-color" | "delete-storage" | "delete-color" | "delete-product";
...
function EditProductMain() {
  ...
  const { isChange, setIsChange, product_name_ascii, id, colors_data, storages_data, detail, combines_data, product_name } = useProductContext();
  // >>> actions
  ...
  const submitAttributes = async () => {
    if (EditGroupRef.current === null) throw new Error("EditGroupRef.current is undefined");
  ...
  const submitDetail = async () => {
    ...
    if (detail === null) {
      ...
      return await privateRequest.put(`${MANAGE_PRODUCT_URL}/details/${detail.id}`, data);
    }
  };
  ...
  const updateDefaultCombine = async (minCB: ProductCombine, prevDefaultCb: ProductCombine | undefined) => {
    ...
    if (!combines_data.length) {
      ...
      if (prevDefaultCb.id !== minCB.id) {
        ...
        await privateRequest.put(`${MANAGE_PRODUCT_URL}/combines/${prevDefaultCb.id}`, { default: false }, { headers: { "Content-Type": "application/json" } });
  ...
  const renderModal = useMemo(() => {
    ...
    switch (addItemModalTarget.current) {
      case "add-color":
        return <AddItem loading={apiLoading} title={"Add color"} cbWhenSubmit={(value) => handleAddColor("Add", value)} setIsOpenModal={setIsOpenModal} />;
      case "edit-color":
      ...
      case "add-storage":
        return <AddItem loading={apiLoading} title={"Add storage"} cbWhenSubmit={(value) => handleAddStorage("Add", value)} setIsOpenModal={setIsOpenModal} />;
      case "edit-storage":
      ...
      case "delete-color":
        ...
        return <ConfirmModal label={`Delete color '${colors_data[curColorIndex.current].color} ?'`} loading={apiLoading} callback={handleDeleteColor} setOpenModal={setIsOpenModal} />;
      case "delete-storage":
        if (curStorageIndex.current === undefined) return <h1 className="text-2xl">Cur index not found</h1>;
        return <ConfirmModal label={`Delete storage '${storages_data[curStorageIndex.current].storage} ?'`} loading={apiLoading} callback={handleDeleteStorage} setOpenModal={setIsOpenModal} />;
      case "delete-product":
        return <ConfirmModal label={`Delete '${product_name}'`} loading={apiLoading} setOpenModal={setIsOpenModal} callback={handleDeleteProduct} />;
    }
  ...
  return (
    <>
      ...
      <div className="pb-[30px]">
        ...
        <EditGroup attrList={attrList} ref={EditGroupRef} />

        <h5 className={classes.label}>Description</h5>
        <div className="rounded-[12px] overflow-hidden">
          <MyEditor content={detail ? detail.content : ""} ref={(ref) => (editorRef.current = ref!)} onUpdate={handleEditorChange} />
        </div>
