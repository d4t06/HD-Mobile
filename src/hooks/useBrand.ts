...
export default function useBrandAction({
    ...
    try {
      switch (props.type) {
        case "Add":
          ...
          // add category
          const catRes = await privateRequest.post(
        ...
        case "Edit":
          ...
          const { id, ...restCategory } = _category;

          if (props.curIndex === undefined || !!id)
            throw new Error("missing current index");
          setApiLoading(true);

          await privateRequest.put(
            `${MANAGE_CAT_URL}/categories/${_category.id}`,
            restCategory
          );
    ...
    try {
      switch (type) {
        case "Add":
          ...
          const brandRes = await privateRequest.post(
            `${MANAGE_CAT_URL}/brands`,
            brand
          );
          ...
          setBrands((prev) => ({
            ...prev,
            [curCategory.category_name_ascii]: newBrands,
          }));
          break;
        ...
        case "Edit":
          if (curCategory === undefined || curBrands === undefined)
            throw new Error("Current not found");
          if (curIndex === undefined || !brand.id)
            throw new Error("missing current index");
          setApiLoading(true);

          await privateRequest.put(
            `${MANAGE_CAT_URL}/brands/${brand.id}`,
            brand
          );
          ...
          setBrands((prev) => ({
            ...prev,
            [curCategory.category_name_ascii]: _newBrands,
          }));
      }
  ...
  const updateCategoryAttrs = (
    ...
    if (curCategory.attributes == undefined)
      throw new Error("Attributes is undefined");
    const newCategories = [...categories];

    const index = newCategories.findIndex(
      (c) => c.category_name_ascii === curCategory.category_name_ascii
    );
  ...
  const updateCategoryPriceRange = (
    ...
    if (curCategory.price_ranges == undefined)
      throw new Error("Price ranges is undefined");
    const newCategories = [...categories];
      ...
      switch (type) {
        case "Add":
          ...
          const newAttributeOrder =
            curCategory.attribute_order + `_${attribute.attribute_ascii}`;

          const res = await privateRequest.post(
            `${MANAGE_CAT_URL}/attributes`,
            attribute
          );
          await privateRequest.put(
            `${MANAGE_CAT_URL}/categories/${curCategory.id}`,
            { attribute_order: newAttributeOrder } as Partial<Category>
          );

          const newAttrData = res.data.data as CategoryAttribute;
          const newCategoryAttrs = [...curCategory.attributes, newAttrData];

          const newCategories = updateCategoryAttrs(
            categories,
            newCategoryAttrs,
            curCategory
          );
        ...
        case "Edit":
          ...
          await privateRequest.put(
            `${MANAGE_CAT_URL}/attributes/${attribute.id}`,
            attribute
          );

          await privateRequest.put(
            `${MANAGE_CAT_URL}/categories/${curCategory.id}`,
            { attribute_order: _newAttributeOrder } as Partial<Category>
          );
          ...
          const _newCategories = updateCategoryAttrs(
            categories,
            curAttrs,
            curCategory
          );
    ...
    try {
      ...
      if (curAttributeOrder.includes(`_${targetAttr.attribute_ascii}`)) {
        newAttributeOrder = curAttributeOrder.replace(
          `_${targetAttr.attribute_ascii}`,
          ""
        );
      } else
        newAttributeOrder = curAttributeOrder.replace(
          `${targetAttr.attribute_ascii}_`,
          ""
        );

      setApiLoading(true);
      await privateRequest.delete(
        `${MANAGE_CAT_URL}/attributes/${targetAttr.id}`
      );
    ...
    try {
      switch (type) {
        ...
        case "Edit":
          ...
          await privateRequest.put(
            `${MANAGE_CAT_URL}/price-ranges/${price.id}`,
            price
          );
  ...
  const deletePriceRange = async (curIndex?: number) => {
    try {
      ...
      setApiLoading(true);
      await privateRequest.delete(
        `${MANAGE_CAT_URL}/price-ranges/${targetPriceRange.id}`
      );

      const newPriceRanges = curPriceRange.filter(
        (b) => b.id !== targetPriceRange.id
      );
      const newCategories = updateCategoryPriceRange(
        categories,
        newPriceRanges,
      ...
      setCategories(newCategories);
      setSuccessToast(
        `Delete price range '${targetPriceRange.label}' successful`
      );
    } catch (error) {
      console.log({ message: error });
