...
const CAT_ATTR_URL = "/app/category_attributes";
...
export default function useBrandAction({
    ...
    try {
      switch (type) {
        case "Add":
          ...
          const newAttrData = res.data.data as Brand;
          const newAttrs = [...curCategory.attributes, newAttrData];
          const newCategory = { ...curCategory, attributes: newAttrs } as Category;

          setCategories((prev) => ({
            ...prev,
            [curCategory.category_ascii]: newCategory,
          }));
          break;
  ...
  const deleteCategoryAttribute = async (curIndex?: number) => {
    try {
      if (curIndex === undefined || curBrands === undefined || curCategory === undefined)
        throw new Error("no have id");
      const targetBrand = curBrands[curIndex];
