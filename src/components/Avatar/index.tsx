...
export default function Avatar({ revert }: { revert?: boolean }) {
   ...
   const decode: Decode = useMemo(() => (auth?.token ? jwtDecode(auth.token) : initRole), [auth]);
...
export function AvatarPlaceholder({ firstChar, image_url }: { firstChar: string; image_url?: string }) {
   return (
