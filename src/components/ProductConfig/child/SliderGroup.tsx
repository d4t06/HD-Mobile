...
function SliderGroup(
  ...
  const trackingSliderImages = () => {
    ...
    if (!initSlider.images.length) newSliderImages = sliderImages;
      ...
      sliderImages.forEach((UKImage) => {
        const exist = initSlider.images.find(
          (existImage) => existImage.image_url === UKImage.image_url
        );
  ...
  const submit = async () => {
    try {
      ...
      let sliderDataToReturn:
        | (SliderSchema & { id: number; color_ascii: string })
        | undefined = undefined;
      ...
      <div className="row">
        {sliderImages.map((sI, index) => (
          <div key={index} className={`col ${width ?? "w-2/12"}`}>
            <Empty className={paddingRatio ?? "pt-[50%]"}>
        ...
        <div className={`col ${width ?? "w-2/12"}`}>
          <Empty
            className={`${error ? "bg-red-200" : ""} ${paddingRatio ?? "pt-[50%]"}`}
