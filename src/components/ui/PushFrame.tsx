...
export default function PushFrame({ children, active, rounded = "rounded-[8px]", type = "border" }: Props) {
   if (type === "border")
      return (
         <div className="bg-[#e1e1e1] p-[4px] pb-[8px] rounded-[14px]">
            <div className="front bg-[#fff] rounded-[12px] overflow-hidden p-[10px]">{children}</div>
         </div>
      );
